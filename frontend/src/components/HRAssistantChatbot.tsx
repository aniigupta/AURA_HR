...
import { 
  ...
  RotateCcw, ShieldCheck, ChevronDown, BookOpen, AlertCircle
} from "lucide-react";
...
export default function HRAssistantChatbot() {
    ...
    setMessages([
      {
        id: `welcome-${Date.now()}`,
        role: "assistant",
        content: `Conversation reset! How can I assist you with **${companyName}** policies today, **${firstName}**?`,
        sources: ["AuraHR AI"],
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }
    ]);
  ...
  const renderFormattedContent = (content: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      if (line.trim().startsWith("### ")) {
        return (
          <h4 key={idx} className="font-bold text-xs text-slate-900 mt-2 mb-1">
  ...
  const formatInline = (text: string) => {
    ...
    // Code inline: `code`
    formatted = formatted.replace(/`([^`]+)`/g, '<code class="bg-slate-100 px-1 py-0.5 rounded text-[11px] font-mono text-indigo-700">$1</code>');
    return formatted;
  };
      ...
      <div className="fixed bottom-5 right-5 z-50">
        {!isOpen ? (
          <button
            ...
            className="group flex items-center gap-2.5 px-4 py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white rounded-full shadow-xl hover:shadow-2xl transition-all duration-200 cursor-pointer transform hover:-translate-y-0.5 active:translate-y-0"
          ...
          >
            ...
            <span className="text-xs font-bold tracking-wide pr-1">Ask HR Policy AI</span>
          </button>
        ) : null}
      </div>
            ...
            <div className="flex items-center gap-1.5 whitespace-nowrap">
              {suggestionPrompts.map((prompt, idx) => (
                <button
                  ...
                  disabled={isLoading}
                  className="text-[11px] px-2.5 py-1 rounded-full bg-slate-100 hover:bg-indigo-50 hover:text-indigo-700 text-slate-600 font-medium transition-colors cursor-pointer shrink-0 border border-slate-200"
                >
                  {prompt}
                </button>
              ))}
          ...
          <div className="p-3 border-t border-slate-200 bg-white shrink-0">
            ...
            >
              <input
                ...
                disabled={isLoading}
                className="flex-1 px-3.5 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all text-slate-800 placeholder:text-slate-400"
              />
              <Button
                type="submit"
                size="sm"
                disabled={isLoading || !inputMessage.trim()}
                className="h-8.5 px-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-semibold shrink-0 cursor-pointer"
              >
